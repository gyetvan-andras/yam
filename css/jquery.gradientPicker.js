...
(function( $ ) {
	...
	GradientSelection.prototype = {
		...
		// removeControlPointIfNeeded: function(ctrlPt) {
		// 	var lookfor = -1;
		// 	if(ctrlPt.position === 0) lookfor = 0;
		// 	else if (ctrlPt.position === 1) lookfor = 1;
		// 	if(lookfor === -1) return;
		// 	var cpidx = this.controlPoints.indexOf(ctrlPt);
		// 	var rmidx = -1;
		// 	for(var idx = 0; idx < this.controlPoints.length;idx++) {
		// 		if(idx === cpidx) continue;
		// 		if(this.controlPoints[idx].position === lookfor) {
		// 			rmidx = idx;
		// 			break;
		// 		}
		// 	}
		// 	if(rmidx != -1) {
		// 		console.log("cp removed:"+ctrlPt.position);
		// 		this.controlPoints.splice(cpidx, 1);
		// 		ctrlPt.$el.remove();
		// 	}
		// },
	...
	function ControlPoint($parentEl, initialState, orientation, listener) {
		...
		this.$cp = $.pixicolors(this.$elcp,this.$el.get(0), opts);
		...
		// var bcr = $parentEl.get(0).getBoundingClientRect();
		// var bounds = [bcr.left,bcr.top, bcr.right, bcr.bottom];
		this.$el.draggable({
